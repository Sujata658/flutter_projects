...
node_labels = {node: stops[int(node) - 1]['name'] for node in graph.nodes}
nx.draw_networkx_labels(graph, pos, labels=node_labels,
                        font_size=8, font_color='black')

# Draw edges
nx.draw(graph, pos, with_labels=False, node_size=100, node_color='lightblue')

# Add labels to edges
for edge, label in labels.items():
    x, y = (pos[edge[0]] + pos[edge[1]]) / 2
