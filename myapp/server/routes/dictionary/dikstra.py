...
def dijkstra(graph, start_stop, end_stop):
    ...
    path = nx.shortest_path(graph, source=start_stop,
                            target=end_stop, weight='label')

    # Calculate the total distance
    total_distance = sum(int(graph[path[i]][path[i + 1]]['label'])
...
# Example usage
start_stop = '1'  # Replace with the actual start stop ID
end_stop = '8'    # Replace with the actual end stop ID

shortest_distance, stops_in_between = dijkstra(graph, start_stop, end_stop)

print(f"Shortest Distance from {node_labels[start_stop]} to {
      node_labels[end_stop]}: {shortest_distance} units")
print("Stops in Between:", [node_labels[stop] for stop in stops_in_between])
