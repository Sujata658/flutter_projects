...
def create_route_graph(graph, routes):
    for route in routes:
        stops_list = route['stops_list']
        graph.add_nodes_from(stops_list)

        # Connect adjacent stops in the route
        for i in range(len(stops_list) - 1):
            graph.add_edge(
                stops_list[i], stops_list[i + 1], route_id=route['id'])

    return graph


def visualize_combined_graph(graph):
    pos = nx.spring_layout(graph)  # Choose a layout for better visualization

    # Draw nodes
    nx.draw_networkx_nodes(graph, pos, node_size=700, node_color='lightblue')

    # Draw edges with different colors for each route
    for route_id in set(nx.get_edge_attributes(graph, 'route_id').values()):
        edges = [(u, v) for (u, v, d) in graph.edges(
            data=True) if d.get('route_id') == route_id]
        nx.draw_networkx_edges(graph, pos, edgelist=edges,
                               label=f'Route {route_id}', width=2)
...
# Assuming stops_data contains your stops JSON
stops_data = [
    {
        "_id": {"$oid": "65c0fc390e141156fc3320fd"},
        "id": 1,
        "name": "Lagankhel",
        "lat": 27.667292191163654,
        "lon": 85.32384361839783
...
# Assuming routes_data contains your routes JSON


graph = create_lat_lon_graph(stops)
# graph = create_route_graph(graph, routes)
visualize_combined_graph(graph)
