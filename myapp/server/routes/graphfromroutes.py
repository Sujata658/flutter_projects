...
def create_routes_graph(routes, stops):
    graph = nx.Graph()
    ...
    for route in routes:
        ...
        for i in range(len(stops_list) - 1):
            ...
            graph.add_edge(stop1, stop2, route_id=route['id'])
...
def visualize_lat_lon_graph(graph):
    ...
    # Add edges (lines) based on routes
    for edge in graph.edges(data=True):
        route_id = edge[2]['route_id']
        # Different color for each route
        edge_color = f'C{int(route_id[-1]) % 10}'
        nx.draw_networkx_edges(graph, pos, edgelist=[(
            edge[0], edge[1])], edge_color=edge_color)

    plt.show()


# Create the stops graph
stops_graph = create_lat_lon_graph(stops)

# Create the routes graph
routes_graph = create_routes_graph(routes, stops)

# Visualize the stops and routes graph
visualize_lat_lon_graph(stops_graph)
